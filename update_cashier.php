...
if(isset($_GET['username'])){
	$users = $_GET['username'];
	$sql = $con->query("SELECT * FROM `doctor` WHERE `username` = '$users'");
...
if($sql = $con->query("UPDATE `doctor` SET `first_name` ='$fname', `last_name` ='$lname', `staff_id` ='$sid', `postal_address` ='$postal', `phone` ='$phone',`email` =  '$email', `username` ='$username', `password` ='$pas' WHERE `username` = '$username'")){
...
<body>
<?php require 'incs/data.php'; ?>
<div class="main_body container mt-4">
	<?php echo $msg; ?>
	<div class="card">
		<div class="card-header">
			<h4><span class="fa fa-stethoscope"></span> Update Doctor</h4>
		</div>
		<div class="card-body">
